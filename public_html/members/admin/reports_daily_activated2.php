					...
					<table width="100%" class="table table-striped table-bordered table-hover" id="historyTable">
						...
						<tbody>
							...
							if(isset($_POST['pin_history'])){
                                    ...
                                    $to = $_POST['to'];
                                $from = $_POST['from'];
                                $con_to = date("Y-m-d", strtotime($to));
                                $con_from = date("Y-m-d", strtotime($from));
                                
								$h=mysqli_query($con,"select date_activated,UNIX_TIMESTAMP(date_activated) as unix ,COUNT(*) as userCount from user WHERE UNIX_TIMESTAMP(date_activated) >UNIX_TIMESTAMP('$con_from') and UNIX_TIMESTAMP(date_activated) < UNIX_TIMESTAMP('$con_to') GROUP BY date_activated order by date_activated asc");
