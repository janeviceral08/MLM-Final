...
$get_userid = mysqli_query($con,"select * from user where account='$userid'");
	...
	echo '<script>alert("Successfully Distribute Income.");window.location.assign("view-pin-request.php");</script>';	
}
              ...
              <!-- /.row -->
                ...
                <div class="col-lg-12">
					...
					<table width="100%" class="table table-striped table-bordered table-hover" id="historyTable">
						...
						<tbody>
					 <?php
									$query = mysqli_query($con,"select * from user where is_active='request'");
									if(mysqli_num_rows($query)>0){
										$i=1;
									...
									?>
                                    	<tr>
                                        	<td colspan="16" align="center">You have no request approval.</td>
                                        </tr>
                                    <?php
									}
								?>
