<div class="col-lg-4">
	...
	<?php
		$me=mysqli_query($conn,"select * from chat_member left join chatrooms on chatrooms.chatroomid=chat_member.chatroomid where chat_member.userid='".$_SESSION['id']."' order by chatrooms.date_created desc");
	...
	?>
		...
		<table width="100%" class="table table-striped table-bordered table-hover" id="myChatRoom">
			...
			<tbody>
			<?php
				$my=mysqli_query($conn,"select * from chat_member left join chatrooms on chatrooms.chatroomid=chat_member.chatroomid where chat_member.userid='".$_SESSION['id']."' order by chatrooms.date_created desc");
					while($myrow=mysqli_fetch_array($my)){
						...
						<tr>
							...
							<td>
								<?php
									$memb=mysqli_query($conn,"select * from chatrooms where userid='".$_SESSION['id']."' and chatroomid='".$myrow['chatroomid']."'");
