...
<?php
    ...
    $query_store = mysqli_query($con,"SELECT *, COUNT(product) as prods  FROM `store_product` where user_email = '$email'");
...
if(is_uploaded_file($_FILES['userImage']['tmp_name'])) {
	...
	$sql = "INSERT INTO store_product(store_uid,product,price,quantity,description,date_added,user_store_name,images_img,from_user,user_email,cat, imageType)
	VALUES('$generated_pin','$prod','$price','$qty','$desc','$date','$store_name', '{$imgData}','user','$store_email','$cat','{$imageProperties['mime']}')";
	$current_id = mysqli_query($con, $sql) or die("<b>Error:</b> Problem on Image Insert<br/>" . mysqli_error($con));
	if(isset($current_id)) {
        echo '<script>alert("Pin request sent successfully");window.location.assign("store.php");</script>';
	
    }
    else{
        //echo mysqli_error($con);
        echo '<script>alert("Unknown error occure.");window.location.assign("add_prod.php");</script>';
    }
}
else{
			...
			<div class="container">
				...
				<div class="row">
					...
					<div class="billing-details">
							...
							<form action=" " method="post" enctype="multipart/form-data" >
								...
								<input class="input" type="text" name="store_name" value="<?php echo $store?>" readonly>
							</div>
									<div class="form-group"> Store Email:
								<input class="input" type="text" name="store_email" value="<?php echo $email?>" readonly>
							</div>
								
								<div class="form-group"> Product Name:
								<input class="input" type="text" name="prod" placeholder="Product Name" required>
							</div>
								<div class="form-group"> Price: 
								<input class="input" type="number" step="0.01" name="price" placeholder="Price" required>
							</div>
								<div class="form-group">Quantity
									<input class="input" type="number" name="qty" placeholder="Quantity" required>
                ...
                ?> 
					...
					<div class="order-notes"> Description:
							<textarea class="input" name="des" placeholder="Description"></textarea>
						</div>
							<div class="form-group">Upload Image:
								<input class="input" type="file" name="userImage" placeholder="Upload Image" required>
							</div>
						</div>
						<!-- /Billing Details -->
