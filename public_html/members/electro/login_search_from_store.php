...
<style type="text/css">
    ...
    $row_prod = mysqli_fetch_array($query_prod);
							...
							?>
													...
													$searchs= $_GET['search'];
														$store= $_GET['store'];
    $query_prod = mysqli_query($con,"SELECT *, COUNT(product) as prods  FROM `store_product` where product LIKE '%$searchs%' and status = 1 and user_store_name	= '$store'");
    $row_prod = mysqli_fetch_array($query_prod);
				...
				<div class="row">
					<div class="col-md-12">
						<ul class="breadcrumb-tree">
							...
							<li class="active"><?php echo $_GET['store'];?> (<?php echo $row_prod['prods']; ?> Products Result)</li>
						</ul>
					</div>
				</div>
				<!-- /row -->
	...
	<?php 
					$search= $_GET['search'];
										$h=mysqli_query($con,"select * from store_product where product LIKE '%$search%'  and status = 1 and user_store_name	= '$store'");
